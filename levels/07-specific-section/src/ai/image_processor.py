...
from src.infrastructure.config import TARGET_SIZE, RENDER_SCALE
...
class ImageProcessor:
    ...
    def __init__(self, target_size: int = TARGET_SIZE) -> None:
        """Initialize image processor.

        Args:
            target_size: Target size for the processed image (square canvas)
        """
    ...
    def process_page(self, page: pymupdf.Page) -> Tuple[str, float, float, float, float]:
        ...
        log.info(
            f"Processed page: {int(original_width)}x{int(original_height)} -> "
            f"{new_size[0]}x{new_size[1]} (scale={scale:.3f})"
        )
    ...
    def denormalize_coordinates(
        ...
        if scale_x == 0:
            log.warning("Scale X is zero, using scale of 1.0")
            scale_x = 1.0

        x0, y0, x1, y1 = [float(v) for v in rect]
        back_scale = 1.0 / scale_x
